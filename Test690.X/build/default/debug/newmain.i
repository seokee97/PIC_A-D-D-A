...
void UART_SendBitsAsChars(unsigned char value) {
...
adcData ADC_Read() {
    ...
    result.adreshData = (unsigned short)(ADRESH);
    result.adreslData = (unsigned short)(ADRESL);
    return result;
}
...
void MCP4921_SetOutput(adcData data) {
    unsigned char configBit = 0b00100000;
    unsigned char highByte = configBit + (unsigned char)data.adreshData;
    unsigned char lowByte = (unsigned char)data.adreslData;
    printf("highByte Value : ");
    for(int i = 8; i >= 0; i--) {

        printf("%d", (highByte >> i) & 1);
    }
    printf("\n");

    printf("lowByte Value : ");
    for(int i = 8; i >= 0; i--) {

        printf("%d", (lowByte >> i) & 1);
    }
    printf("\n");
...
void main(void) {
    ...
    while(1) {
        adcValue = ADC_Read();
        adcValue.adreshData = 0b00000010;
        adcValue.adreslData = 0b11011010;
        MCP4921_SetOutput(adcValue);
        _delay((unsigned long)((100)*(20000000/4000.0)));
    }
}
